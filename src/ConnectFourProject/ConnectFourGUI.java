...
public class ConnectFourGUI {
  ...
  //creating the game board 
    ...
    public ConnectFourGUI(int gameCount) {
...
//    	panel.removeAll();
    	...
    	if(gameCount==0){
        	try {
     		   ...
     		   System.out.println("Sleeping...on new game");
    		   Thread.sleep(1000L);     
    		}
    		catch (Exception e1) {} 
    	}
        ...
        for (int i = 0; i < numCols; i++) {
            ...
            System.out.println("Creating column " + i);
...
//            btnColSelect[i].add(drop);
        ...
        for (int row = 0; row < 6; row++) {
            ...
            for (int column = 0; column < 7; column++) {
                System.out.println("Creating label " + row + ", " + column);
...
//        frame.setContentPane(newPanel);
        System.out.println("Frame: "+ frame+ "Panel: "+panel);
    ...
    public void displayWin(int humanWins, int cpuWins) 
    {
        ...
        int newGameMsgBox = JOptionPane.showConfirmDialog(frame,"\nPlayer 1:  " + humanWins + " win(s)\nPlayer 2:  " + cpuWins + " win(s)\n\nWant to try again?",sWin,JOptionPane.YES_NO_OPTION);        
        if (newGameMsgBox < 1) 
        {
//        	frame.dispose();           
        	newGame = true;
